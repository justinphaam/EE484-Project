...
# Set up the plot
fig, ax = plt.subplots()
im = ax.imshow(temperature_data, cmap=cm.jet, vmin=10, vmax=50)  # Set vmin and vmax to the expected temperature range
ax.set_title('Thermal Image')
cbar = fig.colorbar(im, ax=ax)
cbar.set_label('Temperature (°C)')

# Set up the animation
ani = FuncAnimation(fig, update, blit=False, interval=100, frames=None, cache_frame_data=False)  # Update every 100 ms
